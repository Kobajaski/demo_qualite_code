...
#     assert all(
#         len(data) == len(filters) + 1 and list(filter(lambda x: x != "id", data.keys())) == filters
#         for data in datas
#     )

#     response = client.post(
#         "/list/append", json=[{k: v for k, v in DATA[0].dict().items() if k != "id"}]
#     )
#     assert response.status_code == 200
...
def test_item():
    ...
    response = client.post(
        "/items/append", json={k: v for k, v in DATA[0].dict().items() if k != "id"}
    )
    assert response.status_code == 200
    # assert response.json().get("state") == "success"
    # assert response.json().get("count") == 1
